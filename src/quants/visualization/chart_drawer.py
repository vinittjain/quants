...
class ChartDrawer:
    ...
    def draw_chart(
        ...
        if "close_time" in data.columns:
            data["date"] = pd.to_datetime(data["close_time"], unit="ms", utc=True).dt.tz_convert(
                self.sg_tz
            )
        ...
        fig, (ax1, ax2, ax3) = plt.subplots(
            3, 1, figsize=(12, 16), sharex=True, gridspec_kw={"height_ratios": [3, 1, 1]}
        )
        ...
        # Save the plot
        filename = (
            f"{symbol}_{interval}_{strategy_id}_{trigger_time.strftime('%Y%m%d_%H%M%S')}.png"
        )
